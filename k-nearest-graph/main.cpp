...
char infile[1000], outfile[1000], input[1000], output[1000], outfile2[1000], output2[1000];
...
int main(int argc, char **argv)
{
    ...
    pdir = opendir(infile);
    if (pdir == NULL) {
        printf("Open directory %s failure!\n", infile);
        exit(-1);
    }
    ...
    while ((pent = readdir(pdir)) != NULL) {
        if (strcmp(pent->d_name, ".") != 0 && strcmp(pent->d_name, "..") != 0) {
            ...
            strcpy(input, infile);
            strcat(input, "/");
            strcat(input, pent->d_name);
            strcpy(output, outfile);
            strcat(output, "/");
            strcat(output, pent->d_name);

        	LargeVis model;
            if (if_embed)
                model.load_from_file(input);
            else
                model.load_from_graph(input);
        	model.run(out_dim, n_threads, n_samples, n_propagation, alpha, n_trees, n_negative, n_neighbors, n_gamma, perplexity, output);
        }
    }
