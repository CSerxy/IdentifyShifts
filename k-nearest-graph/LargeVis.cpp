...
void LargeVis::load_from_file(char *infile)
{
    ...
    vec_name = (char **)malloc(n_vertices * sizeof(char *));
    char v_name[200];
	for (long long i = 0; i < n_vertices; ++i)
	{
        fscanf(fin, "%s", v_name);
        vec_name[i] = (char *)malloc(200 * sizeof(char));
        strcpy(vec_name[i], v_name);
...
void LargeVis::construt_knn(char *filename)
{
	normalize();
	run_annoy();
	run_propagation();
	...
	{
		fprintf(fout, "%s %s ", vec_name[edge_from[p]], vec_name[edge_to[p]]);
        fprintf(fout, "%.6lf\n", edge_weight[p]);
...
void LargeVis::run(long long out_d, long long n_thre, long long n_samp, long long n_prop, real alph, long long n_tree, long long n_nega, long long n_neig, real gamm, real perp, char *filename)
{
	gsl_rng_env_setup();
	...
	if (vec) { clean_graph(); construt_knn(filename); }
//	visualize();
}
